...
import { controlPlayingStateAtom, controlCurrentPlayingAtom, controlLoopStateAtom, controlVolumeAtom } from '@/stores/youtube-controller';
...
const PlayController = () => {
  ...
  const LoopStateIcon = useCallback(() => {
    switch (loopState) {
      case 'once':
        return <LoopOnceIcon className="my-auto text-white cursor-pointer" onClick={toggleLoopState}/>
      case 'all':
        return <LoopIcon className="my-auto text-white cursor-pointer" onClick={toggleLoopState}/>;
      case 'none':
        return <LoopIcon className="my-auto text-gray-800 cursor-pointer" onClick={toggleLoopState}/>;
    }
  }, [loopState]);
  ...
  const volumeBarStyle = { '--volume-percent': `${volume * 100}%`} as React.CSSProperties;
  ...
  return (
    <div className="max-w-[296px] flex gap-[52px] mx-auto my-[21px]">
      ...
      <div className='flex flex-col item-center relative'>
        <VolumeStateIcon className="text-white my-auto cursor-pointer" onClick={toggleVolumeBar}/>
        {isShowVolumeBar && <input
            style={volumeBarStyle}
            className={clsx(styles.volumeBar, 'absolute -rotate-90 w-20 h-1 -left-[28px] -top-[28px] bg-gray-800 rounded-lg color-white')}
            type="range"
            max={1}
            min={0}
            step={0.01}
